...
if __name__ == "__main__":
    ...
    try:
        ...
        # Explicitly define the path for localtunnel
        lt_path = "C:\\Users\\rinab\\AppData\\Roaming\\npm\\node_modules\\.bin"
        lt_executable = os.path.join(lt_path, 'lt')
        
        if not os.path.exists(lt_executable):
            raise FileNotFoundError("localtunnel (lt) executable not found. Ensure it is installed and accessible.")
        
        tunnel_process = subprocess.Popen([lt_executable, '--port', '5000', '--subdomain', 'aliceai'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        tunnel_url = None
        for line in tunnel_process.stdout:
            if b'url' in line:
                tunnel_url = line.decode('utf-8').strip().split(' ')[-1]
                break
        if tunnel_url:
            print(f"\n * localtunnel active at: {tunnel_url}")
            print(" * Share this URL with your friends to let them access your Alice AI")
            print(" * Press CTRL+C to quit\n")
        else:
            print("Failed to establish localtunnel.")
